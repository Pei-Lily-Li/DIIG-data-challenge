...
Generally people who work longer at IBM receive higher income. However, there's also a significant amount of people that earn a lot even with a short amount of working experience at the company. But this is understandable considering that these people might be experienced before (as shown by the secon graph). 

## EMPLOYEE CHURN

**Education level vs. Years At Company**

This might have implications on how is IBm doing in terms of preserving those who have higher education, preventing them from going to other companies -- they might lead fo significant contributions if they can stay at the company longer.

From the graph we can see that most people, no matter what degree they have, stay around 5 years at IBM. However, IBM may want to increase this number a little longer because it usually takes quite a long time to train someone (especially for roles that require a better understanding of the firm) before they can start providing 100% contribution to the company. However, if most people leave work in 5 years, the value of those years of training might not be cost-efficient.
...
model_ResearchSci <- lm(YearsAtCompany ~ as.factor(BusinessTravel) + DistanceFromHome + as.factor(EnvironmentSatisfaction) + as.factor(JobSatisfaction) + YearsSinceLastPromotion + MonthlyIncome, data = ResearchSci)

tidy(churn_model)
tidy(model_sales)
tidy(model_ResearchSci)
...
ggplot(ResearchSci, aes(x = YearsSinceLastPromotion, y = YearsAtCompany)) + 
  geom_point()
```
...
```{r}
B <- IBM %>%
  select(WorkLifeBalance, YearsAtCompany, Gender) %>%
...
ggplot(B, aes(x = Balance, y = YearsAtCompany, color = Gender)) + 
  geom_point()
```
